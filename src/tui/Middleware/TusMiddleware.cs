...
namespace tusdotnet
{
	public class TusMiddleware
	{
        ...
        public Func<Task> Next { get; private set; }

        public TusMiddleware(Func<Task> next, Func<ITusConfiguration> configFactory)
		{
		...
		private async Task HandlePostRequest(HttpContext context)
		{
			...
			var fileName = await tusCreationStore.CreateFileAsync(uploadLength, default(CancellationToken));
		...
		private async Task HandlePatchRequest(HttpContext context)
		{
			...
			var hasLock = await semaphore.WaitAsync(TimeSpan.Zero, default(CancellationToken));
		...
		private bool ShouldHandleRequest(HttpContext request)
		{
			var method = request.Request.Method.ToLower();

			if (!request.Request.Headers.ContainsKey(HeaderConstants.TusResumable) && method != "options")
			{
				return false;
			}
			...
			switch (method)
			{
				...
				case "patch":
					//return !IsExactUrlMatch(request) &&
						 //  request.Request.Uri.LocalPath.StartsWith(_config.UrlPath, StringComparison.InvariantCultureIgnoreCase);
				default:
					return false;
			}
		}

		private string GetFileName(HttpContext request)
		{
		...
		private bool IsExactUrlMatch(HttpContext request)
		{
